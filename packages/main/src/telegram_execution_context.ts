...
export default class TelegramExecutionContext {
	...
	/**
	 * Get the message ID from the current update
	 * @returns message id origin type is number , if empty will return 0
	 */
	private getMessageIdNumber(): number {
		return this.update.message?.message_id ?? 0;
	}

	/**
	 * check message is from creator
	 * @returns true if the message is from the creator
	 */
	isFromCreator() {
		if (!this.update.message?.from.id) {
			return false;
		}
		const botCreatorId = this.bot.getCreatorId();
		if (!botCreatorId) {
			console.warn('bot creatorId not settings , alway false');
			return false;
		}

		return botCreatorId == this.update.message.from.id;
	}

	getCreatorId(): number {
		const botCreatorId = this.bot.getCreatorId();
		if (!botCreatorId) {
			console.warn('bot creatorId not settings');
			return 0;
		}
		return botCreatorId;
	}
	...
	async sendMessage(chat_id: string | number, message: string, options: Record<string, object> = {}) {
		return await this.api.sendMessage(this.bot.api.toString(), {
			chat_id: chat_id,
			text: message,
			parse_mode: 'HTML',
	...
	async forwardMessage(chatId: string | number, options: Record<string, number | boolean> = {}) {
		return await this.api.forwardMessage(this.bot.api.toString(), {
			chat_id: chatId,
			from_chat_id: this.getChatId(),
			message_id: this.getMessageIdNumber(),
			...options,
		});
	}
}
